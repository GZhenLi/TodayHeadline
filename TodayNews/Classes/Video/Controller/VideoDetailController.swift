...
class VideoDetailController: UIViewController {
    ...
    fileprivate lazy var relateTableView: UITableView = {
        ...
        tableView.register(UINib(nibName: String(describing: RelateVideoNewsCell.self), bundle: nil), forCellReuseIdentifier: String.init(describing: RelateVideoNewsCell.self))
    ...
    fileprivate lazy var commentTableView: UITableView = {
        ...
        tableView.delegate = self as? UITableViewDelegate
        tableView.backgroundColor = UIColor.globalBackgroundColor()
        tableView.register(UINib.init(nibName: String.init(describing: NewsDetailImageCommentCell.self), bundle: nil), forCellReuseIdentifier: String.init(describing: NewsDetailImageCommentCell.self))
        tableView.separatorStyle = .none
        return tableView
    ...
    /// 相关新闻头部的容器
    fileprivate lazy var  relateHeaderBackView = UIView(frame: CGRect.init(x: 0, y: 0, width: screenWidth, height: 172))
    /// 评论的头部
    fileprivate lazy var commentheaderBackView = UIView(frame: CGRect.init(x: 0, y: 0, width: screenWidth, height: 640 + 172))
    /// 相关新闻头部
...
//
    ...
    //重置播放器
    fileprivate func resetPlayerManeger() {
...
extension VideoDetailController: UITableViewDelegate, UITableViewDataSource {
    ...
    private func cellClickedEvent(cell : NewsDetailImageCommentCell) {
        //头像按钮点击
        cell.avatarButton.rx.controlEvent(.touchUpInside).subscribe(onNext: { [weak self] in
                self?.setupPushViewController(cell: cell)
        }).addDisposableTo(disposeBag)
        
        // 用户名点击
        cell.authorLabel.rx.controlEvent(.touchUpInside)
