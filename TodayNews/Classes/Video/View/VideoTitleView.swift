...
extension VideoTitleView {
    ...
    fileprivate func setupTitleLabelsFrame(titleLabels: [TitleLabel], titles: [TopicTitle]) {
        for (i , label) in titleLabels.enumerated() {
            ...
            w = (topTitle.name! as NSString).boundingRect(with: CGSize.init(width: CGFloat(MAXFLOAT), height: 0), options: .usesLineFragmentOrigin, attributes: [.font: label.font], context: nil).width
            if i == 0 {
        ...
        scrollView.contentSize = CGSize.init(width: (titleLabels.last?.frame.maxX)! + style.itemMargin * 0.5, height: 0)
    }
